...
(function($, window, document, undefined) {
  ...
  _default.settings = {
    ...
    onNodeInspected: undefined,
    onNodeHidden: undefined,
    onNodeShown: undefined,
  ...
  Tree.prototype.unsubscribeEvents = function() {
    ...
    this.$element.off('nodeInspected');
  ...
  Tree.prototype.subscribeEvents = function() {
    ...
    if (typeof (this.options.onNodeInspected) === 'function') {
      this.$element.on('nodeInspected', this.options.onNodeInspected);
    }

    if (typeof (this.options.onNodeHidden) === 'function') {
      this.$element.on('onNodeHidden', this.options.onNodeHidden);
    }

    if (typeof (this.options.onNodeShown) === 'function') {
      this.$element.on('onNodeShown', this.options.onNodeShown);
    }
  ...
  Tree.prototype.setInitialStates = function(node, level) {
    ...
    $.each(node.nodes, function checkStates(index, node) {
      ...
      // set hidden state; unless set always false
  ...
  Tree.prototype.clickHandler = function(event) {
    ...
    if ((classList.indexOf('detail-icon') !== -1)) {
      this.$element.trigger('nodeInspected', $.extend(true, {}, node));
  ...
  Tree.prototype.buildTree = function(nodes, level) {
    ...
    $.each(nodes, function addNodes(id, node) {
      ...
      // disable detail button on checked nodes
      if (node.state.checked) {
        treeItem.find('button.detail-icon').prop('disabled', true);
      } else {
        treeItem.find('button.detail-icon').prop('disabled', false);
      }
  ...
  /**
     Show a previously hidden given tree node
     @param {Object|Number} identifiers - A valid node, node id or array of node identifiers
     @param {optional Object} options
     */
  ...
  Tree.prototype.findNodes = function(pattern, modifier, attribute) {
    ...
    return $.grep(this.nodes, function(node) {
      const val = _this.getNodeValue(node, attribute);
